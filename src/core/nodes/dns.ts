...
export class DnsResolver {
  ...
  clear(): void {
    for (const q of this.pending.values()) q.timer.cancel();
    this.pending.clear();
    this.cache.clear();
  }
...
export class DnsServer {
  ...
  lookup(rawName: string): Ip | undefined {
    const name = normalizeName(rawName);
    return this.config.records.find((r) => normalizeName(r.name) === name)?.ip ?? this.cache.get(name)?.ip;
  }
  ...
  handle(pkt: Ipv4Packet, srcPort: number, msg: DnsMessage, frameId: number, ctx: NodeContext, emit: Emit): void {
    ...
    if (this.config.upstream && this.config.upstream !== this.iface.ip) {
      const id = ++this.idSeq;
      const timer = ctx.timer(DNS_TIMEOUT * 2, DNS_UPSTREAM_TIMER_TAG, { id });
      ...
      const q: Ipv4Packet = { kind: "ipv4", src, dst: this.config.upstream, ttl: 64, payload: { kind: "udp", srcPort: DNS_PORT, dstPort: DNS_PORT, payload: { kind: "dns", id, op: "query", name } } };
